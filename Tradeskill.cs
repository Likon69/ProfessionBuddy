...
namespace HighVoltz
{
    ...
    public class TradeSkill
    {
        ...
        static TradeSkill()
        {
            // WotLK 3.3.5a — no pattern scan needed; HasSpell uses Lua
        }
    ...
    public class Recipe
    {
        ...
        public void UpdateHeader()
        {
            ...
            if (CraftedItemID == 0)
            {
                ...
                // WotLK 3.3.5a: displayName_lang starts at field 3 (16 locale slots)
                // Scan locale fields to find the first valid string pointer
                for (uint li = 3; li < 19 && string.IsNullOrEmpty(_header); li++)
                {
                    var namePtr = dbRow.GetField<uint>(li);
                    if (namePtr != 0)
                        _header = ObjectManager.Wow.Read<string>(namePtr);
                }
            }
            else
            {
                // we need to iterate through ClientDb.ItemSubClass till we find matching
                WoWCache.InfoBlock cache = StyxWoW.Cache[CacheDb.Item].GetInfoBlockById(CraftedItemID);
                ...
                if (cache != null)
                {
                    ...
                    for (int i = dbTable.MinIndex; i <= dbTable.MaxIndex; i++)
                    {
                        ...
                        {
                            // WotLK 3.3.5a: displayName_lang starts at field 11 (16 locale slots)
                            for (uint li = 11; li < 27 && string.IsNullOrEmpty(_header); li++)
                            {
                                var stringPtr = dbRow.GetField<uint>(li);
                                if (stringPtr != 0)
        ...
        internal void InitTools()
        {
            ...
            {
                _tools.Add(GetTool(spell.RequiresSpellFocus, Tool.ToolType.SpellFocus));
            }
            ...
            // Totem items (e.g. Runed Copper Rod for enchanting)
            for (int i = 0; i < 2; i++)
            {
                if (spell.Totem[i] != 0)
                {
                    _tools.Add(GetTool(spell.Totem[i], Tool.ToolType.Item));
                    break;
                }
            }

            // TotemCategory (Blacksmith hammer, mining pick, etc)
            for (int i = 0; i < 2; i++)
            {
                if (spell.TotemCategory[i] != 0)
                {
                    _tools.Add(GetTool(spell.TotemCategory[i], Tool.ToolType.Totem));
                    break;
                }
            }
        }
        ...
        private string GetName()
        {
            ...
            switch (_toolType)
            {
                case ToolType.SpellFocus:
                    t = StyxWoW.Db[ClientDb.SpellFocusObject];
                    r = t?.GetRow(_index);
                    if (r == null) break;
                    // WotLK: Name_lang starts at field 1 (16 locale slots)
                    for (uint li = 1; li < 17 && name == null; li++)
                    {
                        var ptr = r.GetField<uint>(li);
                        if (ptr != 0)
                            name = ObjectManager.Wow.Read<string>(ptr);
                    }
                    break;
                case ToolType.AreaTable:
                    ...
                    r = t?.GetRow(_index);
                    if (r == null) break;
                    // WotLK: AreaName_lang starts at field 11 (16 locale slots)
                    for (uint li = 11; li < 27 && name == null; li++)
                    {
                        var ptr = r.GetField<uint>(li);
                        if (ptr != 0)
                            name = ObjectManager.Wow.Read<string>(ptr);
